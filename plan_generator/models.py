...
class TrainingPlan(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        weeks = plan_gen.generate_weekly_totals(
            self.end_date, self.end_volume, self.start_volume, self.start_date
        )
        for week in weeks:
...
class Week(models.Model):
    ...
    profile = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="weeks", null=True
    )
    plan = models.ForeignKey(
        TrainingPlan, on_delete=models.CASCADE, related_name="weeks", null=True
    )
    ...
    def save(self, *args, **kwargs):
        ...
        days = plan_gen.generate_daily_distances(
            self.plan_distance, [0, 0.2, 0.2, 0.05, 0.15, 0.3, 0.1]
        )
...
class Day(models.Model):
    ...
    profile = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="days", null=True
    )
    ...
    def foot_dist(self):
        foot_dist = self.activities.filter(
            ...
            | models.Q(activity_type="Hike")
        ).aggregate(models.Sum("distance"))
    ...
    @property
    def ride_dist(self):
        foot_dist = self.activities.filter(activity_type="Ride").aggregate(
            models.Sum("distance")
        )
        if foot_dist["distance__sum"] is not None:
            return foot_dist["distance__sum"]
...
class Activity(models.Model):
    ...
    profile = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="activity"
    )
    ...
    day = models.ForeignKey(
        Day, on_delete=models.CASCADE, related_name="activities", null=True, blank=True
    )
    name = models.CharField(max_length=30, null=True, blank=True)
    activity_type = models.CharField(
        max_length=30, choices=ActivityChoices.choices, default=ActivityChoices.RUN
    )
